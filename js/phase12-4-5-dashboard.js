...
function renderScent(){const m=member(),r=m?.scentDiagnosis||window.UNICA_SCENT16?.getMyResult?.();dashboard(statusCard('scent'),r?`<span class="mini-wide"><small>現在の香り</small><b>${safe(r.scentName||'診断済み')}</b></span><span><small>診断日</small><b>${safe(String(r.diagnosedDate||r.date||'').replaceAll('-','/'))||'最新'}</b></span>`:`<span class="mini-wide"><small>まだ診断していません</small><b>何度でも診断できます</b></span>`)}
...
function renderAll(){renderGame();renderScent();renderPrefecture();renderPassport();renderFlowers()}
