...
function scentMiniBadge(ownerUid){ const profile=memberDirectory.get(String(ownerUid||''))||{}; const mg=profile.mindGarden||{}; const chosen=mg.nameFlowerTypeId||(String(mg.nameFlower||'').startsWith('classic|')?String(mg.nameFlower).split('|')[1]:''); if(chosen){const t=window.UNICA_SCENT16?.typeById?.(chosen),name=t?.name||'名前の花',src=window.UNICA_SCENT16?.scentIconUrl?.(chosen)||`assets/scent-icons/${encodeURIComponent(chosen)}.webp`;return `<span class="scent-mini-badge scent-mini-badge-image" title="${esc(name)}" aria-label="${esc(name)}"><img src="${esc(src)}" alt="" loading="lazy"></span>`;} const scent=profile.scentDiagnosis; if(!scent?.typeId)return ''; const type=window.UNICA_SCENT16?.typeById?.(scent.typeId); const flower=type?.flower||scent.flower||'🌸'; const name=scent.scentName||type?.name||'診断済み'; const src=window.UNICA_SCENT16?.scentIconUrl?.(scent.typeId); return src?`<span class="scent-mini-badge scent-mini-badge-image" title="${esc(name)}" aria-label="${esc(name)}"><img src="${esc(src)}" alt="" loading="lazy"></span>`:`<span class="scent-mini-badge" title="${esc(name)}" aria-label="${esc(name)}">${esc(flower)}</span>`; }
