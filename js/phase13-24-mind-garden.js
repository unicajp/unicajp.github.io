...
const AUDIO_KEY='unicaKokoroBloomAudioV2';
const AudioGame=(()=>{let ctx=null,master=null,bgmGain=null,sfxGain=null,timer=0,step=0,shouldPlay=false;
 function pref(){try{return Object.assign({bgm:true,sfx:true,bgmVolume:1,sfxVolume:1.2},JSON.parse(localStorage.getItem(AUDIO_KEY)||'{}'))}catch{return{bgm:true,sfx:true,bgmVolume:1,sfxVolume:1.2}}}
 ...
 function apply(){if(!ctx)return;const p=pref();bgmGain.gain.setTargetAtTime(p.bgm?Math.min(.78,p.bgmVolume*.34):0,ctx.currentTime,.04);sfxGain.gain.setTargetAtTime(p.sfx?Math.min(.95,p.sfxVolume*.79):0,ctx.currentTime,.02)}
