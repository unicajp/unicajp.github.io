...
function closeTitles(){setOpen($('#passportTitlesModal'),false);unlockBody()}
...
function openTitles(){const list=$('#passportTitlesList');const current=$('#detailTitle')?.textContent?.trim();const rows=titleValues();list.innerHTML=rows.length?rows.map((t,i)=>`<div class="passport-title-item"><span>${i===0?'🏷️':'✨'}</span><div><strong>${String(t).replace(/[<>&]/g,'')}</strong><small>${t===current?'現在設定中':'獲得済み'}</small></div>${t===current?'<em>使用中</em>':''}</div>`).join(''):'<div class="passport-title-empty">獲得した称号はまだありません。</div>';setOpen($('#passportTitlesModal'),true);document.body.classList.add('passport-modal-open','modal-open')}
