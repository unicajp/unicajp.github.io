...
const SCENT_PROGRESS_KEY='unicaScent16ProgressV1';
function readProgress(){try{return JSON.parse(localStorage.getItem(SCENT_PROGRESS_KEY)||'null')}catch{return null}}
function writeProgress(){localStorage.setItem(SCENT_PROGRESS_KEY,JSON.stringify({answers,questionIndex,updatedAt:Date.now()}))}
function clearProgress(){localStorage.removeItem(SCENT_PROGRESS_KEY)}
let answers = [];
let questionIndex = 0;
...
function close(){modal?.classList.remove('is-open');modal?.setAttribute('aria-hidden','true');document.body.classList.remove('member-gate-open');window.__UNICA_DIAG_BACK=null;}
function setDiagBack(fn){window.__UNICA_DIAG_BACK=typeof fn==='function'?fn:null;}
function headerBack(){if(typeof window.__UNICA_DIAG_BACK==='function')return window.__UNICA_DIAG_BACK();if(screen?.querySelector('.scentv2-question-page')){if(questionIndex>0){questionIndex--;return showQuestion();}return showIntro();}if(window.__UNICA_SCENT16_FROM_MIND_GARDEN&&window.UNICA_MIND_GARDEN?.open)return window.UNICA_MIND_GARDEN.open();showIntro();}

function showIntro(){ setDiagBack(()=>window.__UNICA_SCENT16_FROM_MIND_GARDEN&&window.UNICA_MIND_GARDEN?.open?window.UNICA_MIND_GARDEN.open():close()); const r=readResult()||member()?.scentDiagnosis; const t=r?typeById(r.typeId):null; const p=!r?readProgress():null,n=Math.min(p?.answers?.filter(Boolean).length||0,QUESTIONS.length),resume=n>0; screen.innerHTML=`<div class="scentv2-hero"><div class="scentv2-kicker">UNICA SCENT</div><div class="scentv2-bloom">${t?.flower||'✿'}</div><h3>${r?'あなたの香りは、ここに。':'あなたの中にある<br>ひとつの香りを見つけよう。'}</h3><p>${r?`一度見つけた <b>${safe(t?.name||r.scentName)}</b> を、UNICA WORLDでのあなたらしさとして大切にします。`:'16の花の中から、行動や感じ方をもとに、あなたらしい香りを見つけます。'}</p><div class="scentv2-meta"><span>16 SCENTS</span><span>約3分</span><span>${resume?`${n}/${QUESTIONS.length} 回答済み`:'MY SCENT'}</span></div>${r?`<button class="scent16-primary" id="viewScentResult">${t?.flower||'🌸'} MY SCENTを見る</button><div class="scentv2-shortcuts"><button id="viewCompatibility">♡ 相性を見る</button><button id="viewFlowerBook">✿ みんなの花を見る</button></div><button class="scentv2-redo" id="redoScent16Intro">もう一度診断する</button>`:resume?`<div class="mind-resume-box"><b>${n} / ${QUESTIONS.length}問まで回答済み</b><small>前回の続きから再開できます</small></div><button class="scent16-primary" id="resumeScent16">続きから</button><button class="scent16-secondary" id="restartScent16">最初から</button>`:`<button class="scent16-primary" id="startScent16">診断をはじめる</button><button class="scent16-secondary" id="viewFlowerBook">✿ 16種類の香りを見る</button>`}<div class="scent16-count" id="scent16DiagnosisCount">みんなの香りを読み込み中…</div></div>`;
 $('#startScent16')?.addEventListener('click',startDiagnosis); $('#resumeScent16')?.addEventListener('click',resumeDiagnosis); $('#restartScent16')?.addEventListener('click',startDiagnosis);$('#viewScentResult')?.addEventListener('click',()=>showResult(r)); $('#viewCompatibility')?.addEventListener('click',showCompatibility); $('#viewFlowerBook')?.addEventListener('click',showFlowerBook); $('#redoScent16Intro')?.addEventListener('click',startDiagnosis); refreshDiagnosisCount();
}
function startDiagnosis(){clearProgress();answers=[];questionIndex=0;showQuestion();}
function resumeDiagnosis(){const p=readProgress();answers=Array.isArray(p?.answers)?p.answers:[];questionIndex=Math.min(answers.filter(Boolean).length,QUESTIONS.length-1);showQuestion();}
function showCategoryIntro(dim){
 setDiagBack(()=>{if(questionIndex>0){questionIndex--;showQuestion();}else showIntro();});
...
function showQuestion(){
 ...
 screen.querySelectorAll('[data-value]').forEach(b=>b.addEventListener('click',()=>{answers[questionIndex]={dim:q.dim,value:Number(b.dataset.value)};questionIndex++;writeProgress();if(questionIndex>=QUESTIONS.length)return finishDiagnosis();showQuestion();}));
...
async function finishDiagnosis(){
 ...
 await new Promise(r=>setTimeout(r,1800)); currentResult=calculate(); clearProgress(); saveLocal(currentResult);
...
function showResult(result,fresh=false){ if(!result)return showIntro(); setDiagBack(showIntro); const t=typeById(result.typeId)||result,n=profileNotes(t); currentResult={...t,...result}; screen.innerHTML=`<div class="scentv2-result"><section class="scentv2-result-cover"><small>YOUR SCENT</small><div>${t.flower}</div><h3>${safe(t.name)}</h3><p>「${safe(t.meaning)}」</p><span>MY SCENT</span></section><section class="scentv2-story"><h4>あなたらしさ</h4><p>${safe(t.personality)}</p></section><div class="scentv2-two"><section><small>STRENGTH</small><h4>あなたの強み</h4><p>${safe(t.strength)}</p></section><section><small>CAREFUL</small><h4>気をつけたいこと</h4><p>${safe(t.weakness)}</p></section></div><section class="scentv2-guide"><h4>もう少し、自分を知る</h4><div><b>人との距離感</b><p>${safe(n.social)}</p></div><div><b>好きな人には</b><p>${safe(n.love)}</p></div><div><b>落ち込んだとき</b><p>${safe(n.down)}</p></div><div><b>実はこんな一面</b><p>${safe(n.hidden)}</p></div></section><section class="scentv2-stats"><h4>5つの個性</h4>${DIMENSIONS.map(([k,label])=>`<div><span>${label}</span><i><em style="width:${clamp(result.stats?.[k]||60,0,100)}%"></em></i><b>${clamp(result.stats?.[k]||60,0,100)}</b></div>`).join('')}</section><div class="scentv2-actions"><button class="scent16-primary" id="compatScent16">♡ ほかの香りとの相性を見る</button><button class="scent16-secondary" id="shareScent16">シェアカードを作る</button><button class="scent16-secondary" id="backScent16">UNICA SCENTへ戻る</button></div><button class="scentv2-redo" id="redoScent16">もう一度診断する</button></div>`;
...
function showShare(result){ setDiagBack(()=>showResult(result)); const t=typeById(result.typeId)||result;screen.innerHTML=`<div class="scent16-share-preview"><div><small>うにかの匂い16診断</small><div class="flower">${t.flower}</div><h3>${safe(t.name)}</h3><p>花言葉：${safe(t.meaning)}</p></div><p>${safe(result.message||'')}</p><footer>UNICA WORLD</footer></div><button class="scent16-primary" id="nativeShareScent">📤 X・Instagram・LINEへシェア</button><button class="scent16-secondary" id="saveScentCard">画像を保存</button><button class="scent16-secondary" id="copyScentLink">リンクをコピー</button><button class="scent16-secondary" id="backFromShare">結果へ戻る</button>`;
...
$('#openScent16')?.addEventListener('click',open);$('#scent16NavBack')?.addEventListener('click',headerBack);$('#scent16Exit')?.addEventListener('click',close);document.querySelectorAll('[data-close-scent16]').forEach(x=>x.addEventListener('click',close));window.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal?.classList.contains('is-open'))close();});window.addEventListener('unica:firebase-member-restored',updateHome);window.addEventListener('unica:scent-diagnosis-saved',updateHome);updateHome();

window.UNICA_SCENT16={close,typeById,compatibility,compatibilityMatrix:COMPATIBILITY_MATRIX,scentIconUrl,scentIconHtml,getMyResult:()=>readResult()||member()?.scentDiagnosis,getProgress:readProgress,showIntro,startDiagnosis,showResult};
